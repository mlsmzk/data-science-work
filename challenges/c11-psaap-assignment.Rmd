...
-   There are a LOT of variables kept track of:
    -   x, idx, L, W, U_0, N_p, k_f, T_f, rho_f, mu_f, lam_f, C_fp, rho_p, d_p, C_pv, h, I_0, eps_p, avg_q, avg_T, rms_T, T_norm
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   According to the metric of MSE, the cheat fit is most accurate. Whereas the nonphysical model is least accurate.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is an output variable. It's an effective predictor because T_norm is calculated using avg_T linearly (i.e. T_norm = (avg_T - T_f)/T_f = 1/T_f \* avg_T - 1.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No. Since T_norm is computed using the current average (i.e. average of plane fluid temperature points in the dataset),
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is a discrete numeric variable that is independent of other variables in the dataset because it's metadata. It doesn't have any physical meaning, but just indicates the trial number.
...
-   T_f (fluid inlet temp), rho_f (fluid density), k_f (turbulence level), and d_p (particle diameter) seem like the most useful predictors.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   A predicted vs. actual plot should have a slope of roughly 1. As we can see, both just x and my q4 fit are pretty accurate predictors. The x fit might be slightly more accurate.
...
-   How many predictors does the `fit_simple` model need in order to make a prediction? What about your model `fit_q4`?
    -   fit_simple only needs one predictor: x. On the other hand, my fit_q4 has four predictors: d_p, rho_f, k_f, and T_f.
...
To start off, the variables they have allowed to vary are given in the following table.
...
-   How much do you trust your model? Why?
    -   I don't trust my model a ton, but it's also an okay measure. The r-squared value for validation is roughly 67% for both the training and validation sets. This means that the model is not overfitted, but it's also not an incredible predictor of a future data point; realistically it would be wrong a decent proportion of the time. Roughly 2/3 of the output can be "explained" by the model I've given it.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   Prediction. Confidence intervals estimate a statistic about a population--prediction intervals predict a statistic about a future member of the population.
-   What fraction of validation cases lie within the interval you predict? How does this compare with `pr_level`?
    -   5% of the validation cases lie within the interval I predict. If the model were predicting the behavior of future cases well, it would probably be roughly 80%. However because the proportion of validation cases in the interval is so low, it is likely that my model is not adequately capturing the data.
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   Between 1.45685 and 2.296426
-   Are there any other recommendations you would provide?
    -   It's a good idea to include safety factors to ensure the expected T_norm will never get dangerously close to the upper or lower bounds of its range.
