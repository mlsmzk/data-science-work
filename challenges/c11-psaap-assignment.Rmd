...
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is a continuous numeric variable (dependent as well). It's an effective predictor because T_norm is calculated using avg_T linearly (i.e. T_norm = (avg_T - T_f)/T_f = 1/T_f \* avg_T - 1.
...
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is a discrete numeric variable that is independent of other variables in the dataset. It doesn't have any physical meaning, but just indicates the trial number.
