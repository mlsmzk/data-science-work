...
-   What value of `fr` did you predict?
    -   1/2, about half of the points in a normal distribution will fall in A since it is cut in half and there is no bias in where the point is generated.
...
p0 <- tibble(z = seq(-3, 0, length.out = 1000)) %>%
  mutate(p = pnorm(z))
```
...
pI <- NA_real_
pI <- integrate(dnorm(mean=0, sd=1), -Inf, -1.96) - integrate(dnorm(mean=0, sd=1), -Inf, 1.96)
pI
```
