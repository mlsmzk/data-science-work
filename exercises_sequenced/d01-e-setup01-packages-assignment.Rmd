...
`install.packages()` installs packages that are not yet installed and need only
be called once for distinct packages. `library()` loads packages that are already
installed and should be used regularly to load packages.
