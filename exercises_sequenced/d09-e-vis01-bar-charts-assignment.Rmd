...
mpg %>%
  ggplot() +
  geom_col(mapping = aes(x = trans, y = displ))
```
...
**Observations**: - Stacking gives the counts of the highways and doesn't illustrate the true relationship, which is that for each city the highway mpg is between 20 and 31.
...
**Observations**: - Plot 2 gives more information in a compact way because it uses color to illustrate drive, which can be telling of a third relationship rather than busy the graph with vibrance.

**Compare `v1` and `v2`**:

-   Which visualization---`v1` or `v2`---enabled you to make more observations?
-   What was the difference between `v1` and `v2`?
    -   v1 color coded bars by class, which was already in the graph

    -   v2 color coded proportions of each bar by drive, which gave new information
...
**Observations** - Dodge likes to make pickups, most car companies make SUVs and/or midsize vehicles, and compact cars are popular among some companies.
...
```{r q5-task}
mpg %>%
  ggplot(aes(x = cyl), width = 1) +
  geom_bar() +
  facet_wrap(~ manufacturer) # changed to wrap from facet_grid
```
