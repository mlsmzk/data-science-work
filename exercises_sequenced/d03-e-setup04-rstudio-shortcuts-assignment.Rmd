...
* Within the script editor or a chunk
    ...
    * `Alt` + `-` - Inserts the assignment operator
    

    * `Shift` + `Cmd/Ctrl` + `M` - Inserts the pipe operator


    * `Cmd/Ctrl` + `Enter` - Runs the selected part/runs the current line


    * `F1` (Note: on a Mac you need to press `fn` + `F1`) - Show help for function on cursor


	* `Cmd/Ctrl` + `Shift` + `C` - Comments out selected line



* Within R Markdown

    * `Cmd/Ctrl` + `Alt` + `I` - Creates a new code cell


* Within a chunk

    * `Shift` + `Cmd/Ctrl` + `Enter` - Runs code chunk


    * `Ctrl` + `I` (`Cmd` + `I`) - Removes indentation
